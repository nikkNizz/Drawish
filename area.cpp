...
void Area::mousePressEvent(QMouseEvent *event)
{
    ...
    originY = event->pos().y();
    if(!isRubb){
        sizes::shape_x_begin =-1; // prevent setInfo from scaling an image that hasn't been created yet
...
void Area::mouseReleaseEvent(QMouseEvent *event)
{
     if(isRubb){
         ...
         sizes::shape_x_begin = 0; // now allow setInfo to scale the selection image
         isRubb= false;
         delete rubb;
         emit endRubber();
     }
...
void Area::paintEvent(QPaintEvent *event)
{
    ...
    QPixmap zoomPix = mPix.scaled(mPix.width() * sizes::zoomLevel, mPix.height() * sizes::zoomLevel, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    painter.drawPixmap(pRect, zoomPix, pRect);
}
