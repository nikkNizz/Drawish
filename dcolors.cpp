...
void dColors::replaceColors(int opt)
{
     ...
     int diff = 15;
     if(opt > 1){
         diff = QInputDialog::getInt(this, "Drawish", tr("Enter increment/decrement value "), 15, 0, 255);
     }
...
void dColors::on_hexValButton_clicked()
{
    ...
    p_Clipboard->setText(ui->hexValButton->text());
     QMessageBox::information(this,"Drawish", tr("Copied!"));
}
...
void dColors::on_averageBtn_clicked()
{
    ...
    ui->redSpin->setValue( redSum/p);
    ui->greenSpin->setValue(greenSum/p);
    ui->blueSpin->setValue(blueSum/p);
}


void dColors::on_countBtn_clicked()
{
    QImage Img = origPix.toImage();
    int count =0;
    QColor k;
