...
QImage DialogEffects::noise()
{
    ...
    QColor k, kRight, kBottom ;
    int totr, totg, totb, nPix;
    for (int y = 0; y < Img.height(); ++y) {
        for (int x = 0; x < Img.width(); ++x) {
            ...
            if(y+1 < Img.height()){
                kBottom = Img.pixelColor(x, y+1);
                totr += (kBottom.red()*2);
                totg += (kBottom.green()*2);
                totb += (kBottom.blue()*2);
                nPix+= 2;
            }
            totr = totr/nPix;
            totg = totg / nPix;
            totb = totb / nPix;
            Img.setPixelColor(x, y, QColor(totr,totg,totb));
            }
        }
    return Img;
}
...
void DialogEffects::on_blurSlider_2_sliderReleased()
{
    ...
    for (int y = 0; y < Img.height(); ++y) {
            for (int x = 0; x < Img.width(); ++x) {
                ...
                totg = totg/totpix;
                totb = totb/totpix;                
                img2.setPixelColor(x, y, QColor(totr,totg,totb));

                }
            }

        newPix = QPixmap::fromImage(img2);
        ui->labelThumb->setPixmap( newPix.scaled(200,170));
        if(ui->autoUpdateCheck_2->isChecked()){origPix = newPix;}
}
...
{
    int w0 = origPix.width();
    int h0 = origPix.height();
    int div = ui->pixelaze_Slider->value();
    newPix = origPix.scaled(w0/div, h0/div);
    newPix = newPix.scaled(w0, h0);
    ui->labelThumb->setPixmap( newPix.scaled(200,170));
 if(ui->autoUpdateCheck_2->isChecked()){origPix = newPix;}
}
...
void DialogEffects::on_lightUp_button_clicked()
{
    QImage Img = origPix.toImage();
    double rapp = 128.00 / double(origPix.width());
    QColor k;
    int r,g,b;
    double z;
    for (int y = 0; y < Img.height(); ++y) {
        ...
        for (int x = 0; x < Img.width(); ++x) {
            ...
            r = origPix.width()-x;
            z = double(r) * rapp;
            r = qMin(k.red() + int(z), 255);
            g = qMin(k.green() + int(z), 255);
            b = qMin(k.blue() + int(z), 255);
