...
shapeArea::shapeArea(QWidget *parent, double Zoomy)  : QLabel{parent}
{
...
void shapeArea::mouseMoveEvent(QMouseEvent *event)
{
    int x = event->pos().x();
    int y = event->pos().y();
    QString shapeLis="squ.cir.aur.aul.abr.abl.rsq.";
    ...
    if(!pressed){
        ...
        if(whereExp == "move"){
           ...
           sizes::selH = sizes::selH + double(incY);
           sizes::selY = event->globalPosition().y()-diffy;
           ...
           sizes::selW = sizes::selW + double(incX);
           sizes::selX = event->globalPosition().x()-diffx;
           if(shapeLis.contains(sizes::activeShape)){
...
void shapeArea::resetGeometry(bool redraw)
{
    ...
    if(whereExp != "move")  drawSomething();
    if(redraw) drawSomething();
    ...
    if(sizes::activeOperation ==8){
        ...
        if(sizes::activeShape == "squ"){
           ...
           // p.drawRect(sizes::line_width+2, sizes::line_width+2, ww-14-sizes::line_width, hh-14-sizes::line_width);
            ...
            int j = sizes::activeShape.toInt();
            int where = sizes::customShapesCoord.indexOf(j) +1;
            QPolygon poly;
            double p1, p2;
            for(int i=where; i< sizes::customShapesCoord.count(); i+=2){
                if(sizes::customShapesCoord.at(i) < 0) break;
                p1 = double(sizes::customShapesCoord.at(i)/100.00);
                p1 = p1 * double(ww);
                p2 = double(sizes::customShapesCoord.at(i+1)/100.00);
                p2 = p2 * double(hh);
                poly.append(QPoint(p1, p2));
            }
            if(!sizes::shapeToCurve){p.drawPolygon(poly);}
            else{
                int pl = poly.count();
                if(pl < 7){
                    QPoint point = poly.last();
                    for(int i=0; i < 7; ++i){
            ...
            QPainterPath pth;
            pth.moveTo(poly[0]);
            pth.cubicTo(poly[1], poly[2],poly[3]);
            p.drawPath(pth);
            pth.moveTo(poly[3]);
            pth.cubicTo(poly[4], poly[5], poly[6]);
            p.drawPath(pth);
            }
        }
        this->setPixmap(npix);
