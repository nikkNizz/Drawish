...
void curveLineArea::mousePressEvent(QMouseEvent *event)
{
    ...
    QPen pen= createPen();
    if(sizes::activeOperation == 11){  // connected lines
        if(sizes::shape_x_begin == -1 && sizes::shape_y_begin == -1){
            ...
            QPainter p(&tPix);
            p.setRenderHint(QPainter::Antialiasing, sizes::aliasing);  // 5 times
            p.setPen(pen);
    ...
    else if(sizes::activeOperation == 9){ // curve
    ...
    else if(sizes::activeOperation == 10 ){  // conn. curve
    if(countClicks ==0){
            if(sizes::shape_x_begin != -1 && sizes::shape_y_begin != -1 ){
...
{
    if(event->button() == Qt::RightButton) return;
    if(sizes::activeOperation == 7 && countClicks == 1){ // line - arrow
    endPoint = event->pos();
    tPix.fill(QColor(255,255,255,0));
    QPainter p(&tPix);
    p.setRenderHint(QPainter::Antialiasing, sizes::aliasing);
    ...
    if(sizes::isArrow == false){
        if(sizes::roundAngle){
            ...
            iDeg *=5;
            // find line length            
            double hip =sqrt((basisLen*basisLen) + (heightLen*heightLen));
            //-----
            QLineF lf;
            lf.setP1(startPoint);
            lf.setAngle(iDeg);
    ...
    int dim = tPix.width() * tPix.height();
    if(dim > 9000000 && dim < 12000000){
        countShow++;
        if(countShow == 2) countShow =0;
    }
    else if(dim > 12000000){
        countShow++;
        if(countShow == 3) countShow =0;
    }
    else{
        countShow =0;
    }
...
{
    QColor k= sizes::activeColor;
    k.setAlpha(trsp);
    QPen pen1(k);
    pen1.setWidth(sizes::line_width * Zoom);
    if(round) {
        pen1.setCapStyle(Qt::RoundCap);
    }else{
        pen1.setCapStyle(Qt::FlatCap);
    }
    if(sizes::dotLines) pen1.setStyle(Qt::DotLine);
    return pen1;
}
