...
void curveLineArea::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(sizes::isArrow == false){
        ...
        if(sizes::lineAngleIndicator){
            deg = abs(deg-90.00);
            int wx = abs(startPoint.x() - endPoint.x());
            int hx = abs(startPoint.y() - endPoint.y());
            infoDeg->setGeometry(endPoint.x()+18, endPoint.y(), 170, 20);
            infoDeg->setText(QString::number(deg, 'g', 3) + "°  H(" + QString::number(wx) + ") V(" + QString::number(hx) + ")");
        }
    }
    else{
...
QPen curveLineArea::createPen(bool round)
{
    ...
    if(round) pen1.setCapStyle(Qt::RoundCap);
