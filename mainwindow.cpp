    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    version_info = "2.0";
    // 2.0: semitransp. pen; infowidget: correct bug; improved connected curves; freehand selection;
    // zoom standard ; stretch area ; tracer; noise effect; bug text area; paste over file
    // quick pen width
...
QPen MainWindow::configPen(QColor &ncol, int alpha)
{
    ...
    if(jColor == 3 || jColor == 4) pen1.setWidth(1); // nib
...
void MainWindow::draw_shape()
{
    ...
    if(sizes::activeOperation == 8 && sizes::isShapeOn ==false){
        ...
        sPix = sPix.scaled(sPix.width() / zoomLevel, sPix.height() / zoomLevel);
        pai.drawPixmap((sizes::selX / zoomLevel)+2 , (sizes::selY / zoomLevel) , sPix);
        showPix();
        sizes::isShapeOn = false;
        delete shape_area;
        shape_area =NULL;
...
void MainWindow::on_historyCombo_activated(int index)
{
    ...
    if(comboText.startsWith("Selection")){
        ...
        sizes::selW *= zoomLevel;
        sizes::selH *= zoomLevel;
       // if(comboText.endsWith("*f")){
        selectionPix = historyPix[index * 2 + 1];  // 1--5
            createSelection(true);
       // }
      //  else{ createSelection();}
    }
    newImage("pix");
    showPix();
}
