    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    version_info = "1.6";
    // 1.6: improved pixelate ; remember window size; resize curveArea; improved blur effect
    // mac info; grid set color; gradient buttons;rounded square;
...
void MainWindow::drawWithPen(){
    ...
    if(penIndex == 3 || penIndex == 4){
        ...
        for(int i=1; i < m; ++i){
            QPainter pai(&pix);
            pai.setPen(pen);
            pai.drawLine(sizes::shape_x_begin +(difx/(m-i)), sizes::shape_y_begin +(dify/(m-i)), sizes::shape_x_begin, sizes::shape_y_begin );
            pai.setClipRegion(reg);
            pai.end();
        }
    }
