    ...
    , ui(new Ui::MainWindow)
{
...
#ifdef Q_OS_LINUX
    sizes::isLinux = true;
    ui->actionDesktop_shortcut->setText(tr("Add to menu"));
#endif

    ui->textOptionsWidget->setVisible(false);
    ui->sliderWidget->setGeometry(80,0,214,31);
    ui->sliderWidget->setVisible(false);
    ui->lineOptionWidget->setGeometry(296,0,402,31);
    ...
    ui->shapeOptionWidget->setGeometry(296,0,361,31);
    ui->shapeOptionWidget->setVisible(false);
    ui->selectionWidget->setGeometry(80,0,300, 31);
    ...
    // open passed file (from command line)
        if(activePathFile != "" && QFile::exists(activePathFile)){
        QPixmap npix= QPixmap();

        if(activePathFile.endsWith("pdf", Qt::CaseInsensitive) && sizes::isLinux){
             npix = openPdf(activePathFile);
        }else{ // not pdf
             npix.load(activePathFile);
             ui->labelActiveFile->setText( activePathFile);
             addToRecent(activePathFile);
        }
            if(npix.isNull()){  // null image
                QMessageBox::information(this, "Drawish", tr("Unsupported file"));
                removeFromRecent(activePathFile);
                 // set default image to area
                wArea->mPix =QPixmap(sizes::areaWidth-8, sizes::areaHeight-8);
                wArea->mPix.fill(Qt::white);
            }else{   // good image
            wArea->mPix = npix;
            sizes::areaHeight=wArea->mPix.height()+8;
            sizes::areaWidth=wArea->mPix.width()+8;
            PlaceEdges();
            }
        }else{  //  no args
             // set default image to area
    ...
    historyPix.push_back(wArea->mPix);
    ...
    historyCount = 1;
    //  verify custom shpes
    if(QFile::exists(QDir::homePath() + "/Drawish_Data/shapes.txt")){
        ui->custShapeLabel->setText("");
    }
    ...
    if(event->mimeData()->hasUrls()){
            foreach(QUrl url, event->mimeData()->urls()){
                ...
                //open image
                 ...
                 QPixmap npix;
                 if(f.filePath().endsWith(".pdf", Qt::CaseInsensitive) && sizes::isLinux){
                ...
                if(!npix.isNull()) wArea->mPix = npix;
...
void MainWindow::readConfig()
{
    ...
    if(k == "")  k = "6";
    ui->horizontalSlider->setValue(k.toInt());
    ...
    k = midstring(config, "favFonts");
    if(k != ""){
        favFonts = k.split(";");
    }else{
        favFonts.push_back(ui->fontComboBox->currentFont().family());
    }
    ...
    k =midstring(config, "angleStep");
    if(k == "1"){
        ui->actionLine_angle_with_5_deg_step->setChecked(true);
        sizes::roundAngle = true;
    }

    defaultPathDialog = midstring(config, "defaultPathDialog");//***
    setGeometry(myX, myY, myW, myH);
    k = midstring(config, "antialiasing");
    if(k.toInt() == 1 || k == ""){
        ui->actionAnti_aliasing_smooth_lines->setChecked(true);
        sizes::aliasing = true;
    }else{
        ui->actionAnti_aliasing_smooth_lines->setChecked(false);
        sizes::aliasing = false;
    }
     k = midstring(config, "lineRoundCap");
    if(k.toInt() == 1 || k == ""){
        ui->actionLine_round_cap->setChecked(true);
        sizes::lineRound = true;
    }else{
        ui->actionLine_round_cap->setChecked(false);
        sizes::lineRound = false;

    }
    k = midstring(config, "maxUndo");
    maxUndo = k.toInt();
    if(maxUndo <1 || maxUndo > 100){
        maxUndo = 25;
    }
    k = midstring(config, "clearText");
    if(k.toInt() == 1){
        ui->actionClean_Text_Area_after_drawing->setChecked(true);
    }
    k =midstring(config, "msg1");
    if(k.trimmed() == "1"){ customShapeMsg = true;}
    k = midstring(config, "angleIndic");
    if(k.trimmed() == "0") ui->actionShow_line_angle_indicator->setChecked(false);
    k = midstring(config, "autoSave");
    if(k.trimmed() ==  "0"){ui->actionSave_to_disk_every_10_changes->setChecked(false); }
}
...
void MainWindow::keyPressEvent(QKeyEvent *ev)
{
    if(sizes::isSelectionOn){
        ...
        pai.end();
        urect= QRect(sizes::selX * sizes::zoomLevel - (48 * sizes::zoomLevel), sizes::selY*sizes::zoomLevel - (48 * sizes::zoomLevel), 96 * sizes::zoomLevel, 96 * sizes::zoomLevel);
        wArea->update(urect);
        sizes::selX= xFin;
        sizes::selY = yFin;
        //showPix();
...
void MainWindow::closeEvent(QCloseEvent *ev)
{
    ...
    val += "<angleIndic>" + QString::number(sizes::lineAngleIndicator) + "</angleIndic>\n"; // ***
    val += "<antialiasing>" + QString::number(sizes::aliasing) + "</antialiasing>\n"; // ***
    val += "<lineRoundCap>" + QString::number(sizes::lineRound) + "</lineRoundCap>\n"; // ***
    val += "<maxUndo>" + QString::number(maxUndo) + "</maxUndo>\n"; // ***
    val += "<clearText>" + QString::number(ui->actionClean_Text_Area_after_drawing->isChecked()) + "</clearText>\n"; // ***

    if(ui->actionSave_to_disk_every_10_changes->isChecked()){
         val += "<autoSave>1</autoSave>\n" ;
    }else{
        val += "<autoSave>0</autoSave>\n" ;
    }

    QString font8 ="";
    for(int i =0; i < favFonts.count(); ++i){
        if(i == 8) break;
        font8 += favFonts.at(i) + ";";
    }
    if(font8.endsWith(";")) font8.chop(1);
    val += "<favFonts>" + font8 + "</favFonts>\n";//***
    if(customShapeMsg ==true){
        val += "<msg1>1</msg1>";
    }else{
        val += "<msg1>0</msg1>";
    }
    if(sizes::roundAngle ==true){
        val += "<angleStep>1</angleStep>";
    }else{
        val += "<angleStep>0</angleStep>";
    }
...
{
    if(sizes::modify ){
        int q = QMessageBox::question(this, "Drawish", tr("Save the image?"), QMessageBox::Yes| QMessageBox::No | QMessageBox::Cancel );
        if(q == QMessageBox::Yes){
            imgSave();
        }
    }
...
{
    if(QFile::exists(f)){
        int q = QMessageBox::question(this, "Drawish", tr("The file already exists, overwrite it?"), QMessageBox::Yes, QMessageBox::No);
        if( q == QMessageBox::No){return false;}
    }
    return true;
}
...
void MainWindow::newImage(QString from, QString path)
{
   ...
   if(from == "zero"){
       sizes::modify= false;
       sizes::areaHeight= (320 * sizes::zoomLevel) + 8;
       sizes::areaWidth= (480 * sizes::zoomLevel) + 8;
      ...
      QPixmap npix;
      if(f.endsWith(".pdf", Qt::CaseInsensitive) && sizes::isLinux){
          npix = openPdf(f);
      }else{
          npix = QPixmap(f);
...
{
        int wh = (ui->horizontalSlider->value() * sizes::zoomLevel) + 2;
...
void MainWindow::updateInfo()
{
  ...
  ui->mouseX_label->setNum(sizes::selX / sizes::zoomLevel);
  ui->mouseY_label->setNum(sizes::selY / sizes::zoomLevel);
  if(sizes::isSelectionOn || sizes::isShapeOn){
    ui->selX_label->setNum(int(sizes::selW / sizes::zoomLevel));
    ui->selY_label->setNum(int(sizes::selH / sizes::zoomLevel));
  }else{
      ui->selX_label->setNum(0);
      ui->selY_label->setNum(0);
  }
  ui->imageW_label->setNum(wArea->mPix.width());
  ui->imageH_label->setNum(wArea->mPix.height());
  int dim = wArea->mPix.width() * wArea->mPix.height();
  ui->imageH_label->setToolTip("Total size: " + QString::number(dim));
  ui->imageW_label->setToolTip("Total size: " + QString::number(dim));
  if(dim > 10000000 && ui->actionShow_button_view->isChecked()){
      ui->ViewportToImg->setVisible(true);
  }else{
...
void MainWindow::untoggle()
{
    ...
    if(!customShapeLoaded){
        customShapeLoaded = true;
        loadCustomShapes();
    }
}

void MainWindow::loadCustomShapes()
{
    // verify the file shapes.txt
    fileIO fio;
    QString custShapes = fio.readFile(QDir::homePath() + "/Drawish_Data/shapes.txt");
    if(custShapes == "") return;
    QString tx= custShapes;
    // remove spaces
    custShapes= custShapes.replace(" ", "");
    custShapes= custShapes.replace("\t", "");
    // remove empty rows and comments
    QStringList d = custShapes.split("\n");
    QStringList d1 = QStringList();
    QString r;
    for(int i=0; i <d.count(); ++i){
        r = d.at(i);
        if(!r.startsWith("//") && r != ""){ d1.push_back(r); }
    }
    d.clear();    
    // check the rules
    ...
    QString validity = "";
    // first name
    if(!d1.at(0).startsWith("@")){
        validity = tr("// The object name must start with @\n");
    }
    ...
    for(int i =0; i < d1.count()-1; ++i){
        r = d1.at(i);
        if(r.startsWith("@")){
            if(r.length() < 2){ validity += tr("// Empty name after @\n");}
            r = d1.at(i+1);
            if(!r.contains(",")){validity += tr("// Coordinate after the name are missing\n"); }

        }else{
            if(!r.contains(",")){validity += tr("// Invalid Coordinates\n"); }
            else{
                d = r.split(",");
                n = d.at(0).toInt(&ok);
                if(!ok){validity += tr("// Integer required in coordinates\n");}
                else{
                    if(n < 0 || n > 100){ validity += tr("// Values ​​between 0 and 100 are required.\n");}
                }
                n = d.at(1).toInt(&ok);
                if(!ok){validity += tr("// Integer required in coordinates\n");}
                else{
    ...
    if(validity != ""){
        fio.createFile(validity + tx , QDir::homePath() + "/Drawish_Data/shapes.txt");
        return;
    }
    // Add points to list
    int numOfShapes =0;
    for(int i=0; i < d1.count(); ++i){
        ...
        if(r.startsWith("@")){
            ...
            d = r.split(",");
            sizes::customShapesCoord.append(d.at(0).toInt());
            sizes::customShapesCoord.append(d.at(1).toInt());
        }
    }
...
void MainWindow::on_redoButton_clicked() // redo
{
    if(!prePix.isNull()){
        if(sizes::isSelectionOn){
            sizes::isSelectionOn=false;
...
void MainWindow::createSelection(bool fromUndo)
{
    ...
    if(sizes::activeOperation != 2){  // !selection area for text
      ...
      ui->selectionWidget->setVisible(true);
      QPixmap blank(sizes::selW/sizes::zoomLevel ,sizes::selH/sizes::zoomLevel);
      blank.fill(Qt::white);
      QPainter p(&wArea->mPix);
      p.drawPixmap(sizes::selX/sizes::zoomLevel ,sizes::selY/sizes::zoomLevel, blank);
      p.end();
...
void MainWindow::on_actionCopy_triggered()
{
    if(sizes::isSelectionOn){
        ...
        p_Clipboard->setPixmap(selectionPix.scaled(sizes::selW / sizes::zoomLevel, sizes::selH / sizes::zoomLevel));
    }
}
...
void MainWindow::on_actionPaste_from_clipboard_triggered()
{
    ...
    if (mimeData->hasImage()) {
        ...
        QPixmap pxm(wArea->mPix.width(), wArea->mPix.height());
...
void MainWindow::pasteImg(QPixmap passedPix)
{
    ...
    if(wi > wArea->mPix.width()){ sizes::areaWidth = wi + 8;}
    if(he > wArea->mPix.height()){sizes::areaHeight = he + 8;}
    reSize();
    sizes::selX =ui->scrollArea->horizontalScrollBar()->value();
    sizes::selY =ui->scrollArea->verticalScrollBar()->value();
    sizes::selH = double(he) * sizes::zoomLevel;
    sizes::selW = double(wi) * sizes::zoomLevel;
...
void MainWindow::on_actionReattach_triggered()
{
    if(remSelX == -123 && remSelY == -145) return;
    if(preCropped.isNull()) return;
    //  --- ok
    save_previous(tr("Reattach"));
    untoggle();
    if(sizes::isSelectionOn){
        sizes::isSelectionOn = false;
        drawCopy();
    }
    // pix must be smaller than preCropped pixmap
    if(remSelX > preCropped.width()-1  || remSelY > preCropped.height()) return;
    QPainter pai(&preCropped);
    pai.drawPixmap(remSelX, remSelY, wArea->mPix);
    wArea->mPix = preCropped;
    remSelX = -123;
    remSelY = -145;
    newImage("pix");
}
...
void MainWindow::on_actionMerge_selection_to_image_triggered()
{
    ...
    QPixmap selectedPix(selectionPix.scaled(sizes::selW / sizes::zoomLevel, sizes::selH / sizes::zoomLevel));
    QPixmap underPix = wArea->mPix.copy(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel, sizes::selW / sizes::zoomLevel, sizes::selH / sizes::zoomLevel);
    QImage selectedImage = selectedPix.toImage();
    QImage underImage = underPix.toImage();
    bool ok;
    int ret = QInputDialog::getInt(this, "Drawish", tr("Selection opacity level"), 50, 1, 99, 1, &ok);
    if(!ok) return;
    ...
    int sw = sizes::selW / sizes::zoomLevel;
    int sh = sizes::selH / sizes::zoomLevel;
...
void MainWindow::on_actionDuplicate_triggered()
{
    if(wArea->mPix.height() * wArea->mPix.width() > 12000000){
        QMessageBox::warning(this, "Drawish", tr("Image too large"));
        return;
    }
    save_previous(tr("Duplicate"));
    addRight(wArea->mPix);
}

void MainWindow::addRight(QPixmap fpix)
{
    int wImg = fpix.width();
    int hImg = fpix.height();
    sizes::areaWidth += (wImg * sizes::zoomLevel) ;
    if(hImg > (sizes::areaHeight / sizes::zoomLevel) - 8){ sizes::areaHeight = (hImg * sizes::zoomLevel) + 8; }
    reSize();
    QPainter p(&wArea->mPix);
    int v= ((sizes::areaWidth / sizes::zoomLevel) - 8 - wImg) + 1 ;
...
void MainWindow::on_actionAdd_bottom_triggered()  // paste from file
{
    ...
    int wImg = fpix.width();
    int hImg = fpix.height();
    sizes::areaHeight += (hImg * sizes::zoomLevel);
    if(wImg > (sizes::areaWidth / sizes::zoomLevel) - 8){ sizes::areaWidth = (wImg * sizes::zoomLevel) + 8; }
    reSize();
    QPainter p(&wArea->mPix);
    int v= ((sizes::areaHeight / sizes::zoomLevel) - 8 - hImg) + 1 ;
...
{
    if(sizes::isSelectionOn){
        QPixmap selPix = selectionPix.scaled(sizes::selW / sizes::zoomLevel, sizes::selH / sizes::zoomLevel);
        QFileDialog dialog(this);
        if(QDir(defaultPathDialog).exists() == false){defaultPathDialog = QDir::homePath();}
        QString f =dialog.getSaveFileName(this, tr("Drawish save image"), defaultPathDialog);
        if(f ==""){return;}
...
void MainWindow::on_favFontsButton_clicked()
{
    QString font= QInputDialog::getItem(this, "Drawish", "Font", favFonts);
    if( font != "") ui->fontComboBox->setCurrentFont(QFont(font));
}
...
{
    if(sizes::activeColor != lastColor && sizes::activeColor != Qt::black && sizes::activeColor != Qt::white && sizes::activeColor != Qt::red && sizes::activeColor != Qt::gray && sizes::activeColor != Qt::blue && sizes::activeColor != Qt::green && sizes::activeColor != Qt::magenta && sizes::activeColor != Qt::cyan && sizes::activeColor != Qt::yellow ){
        ui->lastColor->setStyleSheet("background: " + sizes::activeColor.name());
        lastColor = sizes::activeColor;
    }
...
void MainWindow::on_lastColor_clicked()
{
    set_activeColor(lastColor.red(), lastColor.green(), lastColor.blue());
}
...
{
    // combopen: 0 round, 1 square, 2 highlight, 3 slash, 4 backslash, 5 spray, 6 random 7 pencil


    if(sizes::penType ==6 ){ // random
    ...
    else if(sizes::penType == 15){ // fusion

        int df = sizes::line_width /2;
        int a = (sizes::selX / sizes::zoomLevel) -df;
        int c = (sizes::selY / sizes::zoomLevel) -df;

        QPixmap pxm= wArea->mPix.copy(a, c, sizes::line_width, sizes::line_width );
        ...
        ncol = QColor(redSum/df, greenSum / df, blueSum/df, 180);
    }
    else if(sizes::penType == 11){
        ncol = Qt::white;
    }
    ...
    if(sizes::penType == 3 || sizes::penType == 4 ) pen1.setWidth(1);
    else if(sizes::penType ==1 || sizes::penType == 11){ pen1.setCapStyle(Qt::SquareCap);}
...
void MainWindow::drawWithPen(){
    ...
    // stylus
    if(sizes::penType == 8) return;
    if(sizes::penType == 3 || sizes::penType == 4){
        QPainter pai(&wArea->mPix);
        pai.setRenderHint(QPainter::Antialiasing, sizes::aliasing);
        ...
        QPolygon poly;
        if(sizes::penType == 4){
        poly << QPoint(sizes::shape_x_begin/sizes::zoomLevel + corners, sizes::shape_y_begin/sizes::zoomLevel + corners) << QPoint(sizes::shape_x_begin/sizes::zoomLevel - corners, sizes::shape_y_begin/sizes::zoomLevel - corners) << QPoint(sizes::shape_x_end/sizes::zoomLevel - corners, sizes::shape_y_end/sizes::zoomLevel - corners) << QPoint(sizes::shape_x_end/sizes::zoomLevel + corners, sizes::shape_y_end/sizes::zoomLevel + corners);
        }else{
        poly << QPoint(sizes::shape_x_begin/sizes::zoomLevel + corners, sizes::shape_y_begin/sizes::zoomLevel - corners) << QPoint(sizes::shape_x_begin/sizes::zoomLevel - corners, sizes::shape_y_begin /sizes::zoomLevel+ corners) << QPoint(sizes::shape_x_end/sizes::zoomLevel - corners, sizes::shape_y_end /sizes::zoomLevel+ corners) << QPoint(sizes::shape_x_end /sizes::zoomLevel+ corners, sizes::shape_y_end/sizes::zoomLevel - corners);
        }
        pai.drawPolygon(poly);
        urect = poly.boundingRect();
    }
    else if(sizes::penType == 10){
        colorEraser();
    }
    else if(sizes::penType == 12){
        eraseExcept();
    }
    else if(sizes::penType == 9 && sizes::line_width > 2){       // smudge
        int corners = sizes::line_width / 2;
        QPixmap pxm =wArea->mPix.copy(sizes::shape_x_end/sizes::zoomLevel-corners, sizes::shape_y_end/sizes::zoomLevel-corners, sizes::line_width, sizes::line_width);
        QImage imgOver = freeSelPix.toImage();
        QImage imgUnder = pxm.toImage();
        double r1, r2, g1, g2, b1, b2;
        int posX, posY;
        double strong = 0.0;
        QPixmap npix(sizes::line_width, sizes::line_width);
        npix.fill(Qt::transparent);
        QImage nImg = npix.toImage();
        nImg =nImg.convertToFormat(QImage::Format_ARGB32);

        for(int xx =0; xx < sizes::line_width; ++xx){
            for(int yy =0; yy < sizes::line_width; ++yy){
                posX = sizes::line_width - xx - xx;
                posY = sizes::line_width - yy -yy ;
                double hyp = sqrt((posX * posX) + (posY * posY) );
                if(hyp <= double(sizes::line_width) ){ // you are in the area of circle (more or less)
                    strong = hyp / double(sizes::line_width) *0.8 ;
                    r1 = double(imgOver.pixelColor(xx, yy).red());
                    r2 = double(imgUnder.pixelColor(xx, yy).red());
                    r2 = r2 + (r2 * strong);
                    r1 = r1 + (r1 * (1.0-strong));
                    r1 = (r1 + r2) /3;
                    g1 = double(imgOver.pixelColor(xx, yy).green());
                    g2 = double(imgUnder.pixelColor(xx, yy).green());
                    g2 = g2 + (g2 * strong);
                    g1 = g1 + (g1 * (1.0-strong));
                    g1 = (g1 + g2) /3;
                    b1 = double(imgOver.pixelColor(xx, yy).blue());
                    b2 = double(imgUnder.pixelColor(xx, yy).blue());
                    b2 = b2 + (b2 * strong);
                    b1 = b1 + (b1 * (1.0-strong));
                    b1 = (b1 + b2) /3;
                    nImg.setPixelColor(xx, yy, QColor( r1, g1, b1));
                }

            }
        }
        freeSelPix = QPixmap::fromImage(nImg);
        QPainter pai(&wArea->mPix);
        pai.drawPixmap(sizes::shape_x_end/sizes::zoomLevel-(corners), sizes::shape_y_end/sizes::zoomLevel-(corners), freeSelPix);
        pai.end();
        freeSelPix = wArea->mPix.copy(sizes::shape_x_end/sizes::zoomLevel-corners, sizes::shape_y_end/sizes::zoomLevel-corners, sizes::line_width, sizes::line_width);
        urect= QRect (sizes::shape_x_begin - (50 * sizes::zoomLevel), sizes::shape_y_begin - 50 * sizes::zoomLevel, 100 * sizes::zoomLevel, 100 * sizes::zoomLevel);

    }
    else if(sizes::penType == 13 || sizes::penType == 14){
        int corners = sizes::line_width / 2;
        QPixmap pxm =wArea->mPix.copy(sizes::shape_x_end/sizes::zoomLevel-corners, sizes::shape_y_end/sizes::zoomLevel-corners, sizes::line_width, sizes::line_width);
        QImage img2 = pxm.toImage();
        QColor k;
        for (int y = 0; y < img2.height(); ++y) {
            QRgb *line = reinterpret_cast<QRgb*>(img2.scanLine(y));
            for (int x = 0; x < img2.width(); ++x) {
                QRgb &rgb = line[x];
                double hyp = sqrt((corners - x)*(corners - x) + (corners - y)*(corners-y) );
                k = QColor(rgb);//qRed(rgb), qGreen(rgb), qBlue(rgb)
                if( sizes::penType == 13){
                    if(hyp < double(corners) +1.00){ k = k.lighter(104);}
                    else{ k = k.lighter(102);}
                }
                else if(sizes::penType == 14){
                    if(hyp < double(corners) +1.00 ){k = k.darker(102);}
                    else{ k = k.darker(101);}
                }
                img2.setPixelColor(x, y, k);
            }
        }
        QPainter pai(&wArea->mPix);

        pai.drawPixmap(sizes::shape_x_end/sizes::zoomLevel-(corners), sizes::shape_y_end/sizes::zoomLevel-(corners), QPixmap::fromImage(img2));
        pai.end();
        urect= QRect (sizes::shape_x_begin - (50 * sizes::zoomLevel), sizes::shape_y_begin - 50 * sizes::zoomLevel, 100 * sizes::zoomLevel, 100 * sizes::zoomLevel);

    }

    else if(sizes::penType == 5){
        spray_draw();
    }
    else if(sizes::penType == 7){ // pencil
        QPainter pai(&wArea->mPix);
        pai.setRenderHint(QPainter::Antialiasing, sizes::aliasing);
        int ray = sizes::line_width / 2;
        int nRand;
        QPixmap pencilPix(sizes::line_width, sizes::line_width );
        pencilPix.fill(QColor(255,255,255,0));
        QImage img = pencilPix.toImage();
        int alfa=255;
        int ar = sizes::activeColor.red();
        int ag = sizes::activeColor.green();
        int ab = sizes::activeColor.blue();
        int aar, aag, aab;
        for(int h =0; h < sizes::line_width; ++h){
            for(int w =0; w < sizes::line_width; ++w){
                ...
                if(hyp < double(ray) + 0.51){   // you are in the area of circle
                    ...
                    aag = (ag + nRand) / 2;
                    aab = (ab + nRand) / 2;
                    if(ui->markerButton->isChecked()){alfa = 32;}
                    else{ alfa =255;}
                    img.setPixelColor(w, h, QColor(aar, aag, aab, alfa));
                }
            }
        }
        ...
        foreach (QPoint point, penPath) {
            pai.drawPixmap(point.x()-ray, point.y()-ray , QPixmap::fromImage(img));
        }
        pai.end();
        ...
        if(sizes::penType == 2){ //highlight
          QColor k2 = sizes::activeColor;
           k2.setAlpha(6);
           pen2.setColor(k2);
           pen2.setWidth(sizes::line_width + 8);
           pen2.setCapStyle(Qt::RoundCap);
        }
        if(ui->markerButton->isChecked()){
            int t = 100 / sizes::line_width;
...
void MainWindow::createPenPath()
{
    penPath.clear();
    int x1 = sizes::shape_x_begin / sizes::zoomLevel;
    int y1 = sizes::shape_y_begin / sizes::zoomLevel;
    int x2 = sizes::shape_x_end / sizes::zoomLevel;
    int y2 = sizes::shape_y_end / sizes::zoomLevel;
    int stepx = x2-x1;
    int stepy = y2-y1;
    int maxIter = abs(stepx);
    if(abs(stepy) > maxIter) maxIter = abs(stepy);
    double factorx= double(stepx) / maxIter;
    double factory = double(stepy) / maxIter;
    for(int i=0; i < maxIter; ++i){
        penPath.append(QPoint(x1+factorx*i, y1 + factory*i));
        }
...
void MainWindow::draw_first_point()
{
    ...
    if(sizes::penType > 2 && sizes::penType < 5 ){
        // stylus dont draw 1st point
        save_previous(tr("Stylus pen"));
        return;
    }
    if(sizes::penType == 9){
        save_previous(tr("Smudge"));
        int corners = sizes::line_width / 2;
        freeSelPix = wArea->mPix.copy(sizes::shape_x_begin/sizes::zoomLevel-corners, sizes::shape_y_begin/sizes::zoomLevel-corners, sizes::line_width, sizes::line_width);
        return;
    }
    else if(sizes::penType == 13){ save_previous(tr("Lighten")); return;}
    else if(sizes::penType == 14){ save_previous(tr("Darken")); return;}

    if(sizes::penType == 7){
        save_previous(tr("Pencil"));
        return;
    }
    ...
    if(sizes::penType == 12){
        save_previous(tr("Erase except"));
        eraseExcept();
        return;
    }
    if(sizes::penType == 5){save_previous(tr("Spray"));
        spray_draw();
        return;
    }
    ...
    if(sizes::penType == 8){ // labels
        ...
        QFont fo;
        fo.setPixelSize(22);
        fo.setWeight(QFont::ExtraBold);
        pai.setFont(fo);
        pai.drawLine(sizes::shape_x_begin / sizes::zoomLevel,sizes::shape_y_begin/sizes::zoomLevel, sizes::shape_x_begin / sizes::zoomLevel+ 8, sizes::shape_y_begin / sizes::zoomLevel-8 );
        pai.drawText(sizes::shape_x_begin / sizes::zoomLevel+10, sizes::shape_y_begin / sizes::zoomLevel-10, QString::number(nextLabel));
    }else{
        pen.setWidth(sizes::line_width);
        pai.setPen(pen);
        pai.drawPoint(sizes::shape_x_begin / sizes::zoomLevel, sizes::shape_y_begin/ sizes::zoomLevel);
        urect = QRect(sizes::shape_x_begin - (22 * sizes::zoomLevel), sizes::shape_y_begin - (22 * sizes::zoomLevel), 44 * sizes::zoomLevel, 44 * sizes::zoomLevel);
        wArea->update(urect);
    }
    pai.end();

    sizes::selX = sizes::shape_x_begin / sizes::zoomLevel;
...
{
    int ax = (sizes::shape_x_begin / sizes::zoomLevel) - (sizes::line_width/2);
    int ay = (sizes::shape_y_begin / sizes::zoomLevel) - (sizes::line_width/2);

    int similarity= ui->similaritySlider->value();
    QPixmap pxm = wArea->mPix.copy(ax, ay, sizes::line_width, sizes::line_width );
    QImage img = pxm.toImage();
...
void MainWindow::eraseExcept()
{
    ...
    for(int ix=0; ix < img.width(); ++ix){
        for(int iy=0; iy < img.height(); ++iy){
            if(QColor(img.pixelColor(ix, iy)) != sizes::activeColor){
                img.setPixelColor(ix, iy, Qt::white);
            }
        }
    }
    pxm = QPixmap::fromImage(img);
...
void MainWindow::on_conn_Curve_clicked()
{
    ...
    if(ui->conn_Curve->isChecked()){
        ...
        ui->sliderWidget->setVisible(true);
        int t= 255;
        if(ui->markerButton->isChecked()) t = 128;
        cl_area = new curveLineArea(wArea, sizes::zoomLevel, t);
...
{
    countChanges++;
    if(ui->actionSave_to_disk_every_10_changes->isChecked() && countChanges > 9){
        countChanges =0;
        // save to disk
        wArea->mPix.save(QDir::homePath() + "/Drawish_Data/tmp_pix1.png");
    }
    historyCount = 1;
...
QString MainWindow::selectionCoords()
{
    ...
    return ("Selection*" + QString::number(int(sizes::selX / sizes::zoomLevel)) + "*" + QString::number(int(sizes::selY / sizes::zoomLevel)) + "*" + QString::number(int(sizes::selW / sizes::zoomLevel)) + "*" + QString::number(int(sizes::selH / sizes::zoomLevel)) );
...
void MainWindow::fill_()
{
    ...
    QImage img = wArea->mPix.toImage();
    img =img.convertToFormat(QImage::Format_ARGB32);
    QList<int> coord;
    coord.push_back(sizes::selX / sizes::zoomLevel);
    coord.push_back(sizes::selY / sizes::zoomLevel);
    QColor oldColor= QColor::fromRgb(img.pixel(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel));
    ...
    int similarity= ui->similaritySlider->value();
    if(oldColor == sizes::activeColor && similarity == 0 )return;

    int posx, posy;
    if(similarity != 0 && isSimil(QColor::fromRgb(img.pixel(coord[0], coord[1])), sizes::activeColor, similarity)){
        img.setPixelColor(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel, ncol);
        int mw = img.width();
        int mh = img.height();
        QList<bool> mask(mw * mh, false);
        while(coord.count()>0){
            posx = coord[0];
            posy = coord[1];
            int posMask;
            coord.remove(0,2);
            if(posy-1 > -1 ){
                if(isSimil(QColor(img.pixelColor(posx, posy-1)), oldColor, similarity)){
                    posMask = (posy -1 )* mw + (posx);
                    if(mask[posMask] == false){
                        mask[posMask] = true;
            ...
            if(posx-1 > -1 ){
                if(isSimil(QColor(img.pixelColor(posx-1, posy)), oldColor, similarity)){
                    posMask = (posy * mw) + (posx-1);
                    if(mask[posMask] == false){
                         mask[posMask] = true;
    ...
    wArea->mPix = QPixmap::fromImage(img);
    wArea->update();
...
void MainWindow::fillInsideBlack()
{
    ...
    img.setPixelColor(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel, Qt::black);
    int posx, posy;
...
bool MainWindow::isSimil(QColor k1, QColor k2, int affinity)
{
    ...
    if(diffr > affinity )return false;
    return true;
}
...
// differential redo
void MainWindow::on_diffRedoBtn_clicked()
{
    // prepix is newer
    if(!prePix.isNull()){
        if(sizes::isSelectionOn){
        ...
        int w1 = qMax(prePix.width(), wArea->mPix.width());
        int h1 = qMax(prePix.height(), wArea->mPix.height());
        QPixmap wpix(w1, h1);
        wpix.fill(Qt::white);
        QPainter p(&wpix);
        ...
        // find differences
        QColor k1, k2;
        QImage imgPix= wpix.toImage();
        QImage imgPrePix = prePix.toImage();
        for(int y =0; y < prePix.height(); ++ y){
            for(int x=0; x < prePix.width();++x){
                k1 = imgPix.pixelColor(x, y);
                k2 = imgPrePix.pixelColor(x, y);
                if(k1 == k2){ imgPrePix.setPixelColor(x, y, Qt::white);}
            }
        }
        save_previous(tr("Differential redo"));
        wArea->mPix = QPixmap::fromImage(imgPrePix);
        wArea->mPix = wArea->mPix.copy(0,0, imgPrePix.width(), imgPrePix.height());
        newImage("pix");
    }
}
...
void MainWindow::spray_draw()
{
    ...
    for(int i=0; i< sizes::line_width; ++i){
        ...
        int dy= (QRandomGenerator::global()->generate() % lw )-lw/2;
         ...
         pai.drawPoint((sizes::selX / sizes::zoomLevel) + dx, (sizes::selY / sizes::zoomLevel) + dy);
    }
    urect = QRect(sizes::selX -(sizes::line_width * sizes::zoomLevel), sizes::selY -(sizes::line_width * sizes::zoomLevel), lw * sizes::zoomLevel, lw * sizes::zoomLevel);
    //showPix();
}
...
void MainWindow::get_color()
{
    ...
    if(ui->colorBalanceCheck->isChecked()){
        QColor colorRif = QColor::fromRgba(img.pixel(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel));
    ...
    QColor co = QColor::fromRgba(img.pixel(sizes::selX / sizes::zoomLevel, sizes::selY / sizes::zoomLevel));
        set_activeColor(co.red(), co.green(), co.blue());
        //ui->colorActiveButton->setStyleSheet("background-color:" + sizes::activeColor.name());
    //ui->rgbLabel->setText("Rgb " + QString::number(sizes::activeColor.red()) + " " + QString::number(sizes::activeColor.green())+ " " + QString::number(sizes::activeColor.blue()));
    }
    }
...
{
    sizes::selW = sizes::init_w_shape * sizes::zoomLevel;
    sizes::selH = sizes::init_h_shape * sizes::zoomLevel;
...
void MainWindow::draw_shape()
{
    ...
    if(sizes::activeOperation == 8 && sizes::isShapeOn ==false){
        ...
        sizes::selX /= sizes::zoomLevel;
        sizes::selY /= sizes::zoomLevel;
...
// shape to sel
void MainWindow::on_shapeToSel_clicked()
{
    if(!sizes::isShapeOn) return;
    shape_area->redrawForCopy();
    QPixmap spix = shape_area->pixmap();
    int sx = shape_area->x()/sizes::zoomLevel;
    int sy = shape_area->y()/sizes::zoomLevel;
    int sw = shape_area->width()/sizes::zoomLevel;
    int sh = shape_area->height()/sizes::zoomLevel;
    QImage img = spix.toImage().scaled(sw, sh);
    QImage ipix = wArea->mPix.toImage();
    QColor k;
    for(int y=0; y < img.height(); ++y){
        for(int x=0; x < img.width(); ++x){
            k= img.pixelColor(x, y);
            if( k == sizes::activeColor){
                k = ipix.pixelColor(x+sx, y+sy);
                img.setPixelColor(x, y, k);
                ipix.setPixelColor(x+sx, y+sy, Qt::white);
            }
        }
    }
    selectionPix = QPixmap::fromImage(img);
    ...
    wArea->mPix = QPixmap::fromImage(ipix);
    on_selectionAreaButton_clicked();
...
{
    sizes::line_width = value;
    if(sizes::activeOperation ==3 || sizes::activeOperation == 10){
        wArea->setCursor(rectCursor());
    }
    if(sizes::isShapeOn){
        shape_area->drawSomething();
    }
    ui->labelSlider->setText(QString::number(sizes::line_width));
}
...
void MainWindow::on_shapeButton_clicked()
{
    ...
    if(ui->shapeButton->isChecked()){
        ...
        ui->sliderWidget->setVisible(true);
        ui->shapeOptionWidget->setVisible(true);
        if(!QFile::exists(QDir::homePath() + "/Drawish_Data/shapes.txt") && customShapeMsg == false){
            QMessageBox::information(this, "Drawish", tr("You can add <b>custom shapes</b>. Read the guide on Github."));
            customShapeMsg = true;
        }
    }else{
        sizes::activeOperation = 0;
...
void MainWindow::on_shapesCombo_currentIndexChanged(int index)
{
    ...
    if(index < shapeNames.count()){
        ...
        int n = index - shapeNames.count();
        n++;
        sizes::activeShape = "-" + QString::number(n);
    }
...
void MainWindow::finish_lines()
{
    ...
    if(sizes::isCurveLineAreaOn){
      QPixmap cPix = cl_area->tPix;
      cPix = cPix.scaled((sizes::areaWidth-8) / sizes::zoomLevel, (sizes::areaHeight-8) / sizes::zoomLevel);
      QPainter p(&wArea->mPix);
...
void MainWindow::createFreeSel()
{
    // draw polygon
    QPixmap cPix = cl_area->tPix;
    cPix = cPix.scaled((cPix.width() / sizes::zoomLevel), (cPix.height() / sizes::zoomLevel));
    ...
    QImage aPix = wArea->mPix.toImage();
    ...
    // create selection
    // ---- def sizes
    sizes::selX = x1 * sizes::zoomLevel;
    sizes::selY = y1 * sizes::zoomLevel;
    sizes::selW = double(polyRect.width()) * sizes::zoomLevel ;
    sizes::selH = double(polyRect.height()) * sizes::zoomLevel;
    // ----- check all
    sizes::activeOperation=1;
    ...
    // show pix
    wArea->mPix = QPixmap::fromImage(aPix);
   // showPix();
    // ----- set pixmap    
    selectionRect = new selectionArea(wArea);
    selectionRect->resetGeometry();
...
void MainWindow::finish_curve()
{
    ...
    if(sizes::isCurveLineAreaOn){
      QPixmap cPix = cl_area->tPix;
      cPix = cPix.scaled(wArea->mPix.width(), wArea->mPix.height());
      QPainter p(&wArea->mPix);
      p.setRenderHint(QPainter::Antialiasing, sizes::aliasing);
      p.drawPixmap(0,0, cPix);
      //showPix();
...
void MainWindow::view_zoom()
{
    ...
    if(zoom_area == nullptr){
      ...
      posXofZoom = (sizes::selX / sizes::zoomLevel) -(nSquars/2);
      posYofZoom = (sizes::selY / sizes::zoomLevel) -(nSquars/2);
      if(posXofZoom < 0) posXofZoom = 0;
      if(posYofZoom < 0) posYofZoom = 0;
      QPixmap cPix = wArea->mPix.copy(posXofZoom, posYofZoom, nSquars, nSquars);
      QImage img= cPix.toImage();
      QPixmap zoomPix(nSquars * 14, nSquars * 14);
      QColor k;
      ...
      zoom_area->setGeometry(160, 70 , nSquars * 14, nSquars * 14);
      zoom_area->show();
      raiseBorders();
    }
    else{
        // copy zoomed image
        if(sizes::zoomEdited){ save_previous(tr("Zoom"));}
        sizes::zoomEdited = false;
        QImage img = zoom_area->pixmap().toImage();
        QPixmap cPix(nSquars, nSquars);
        QColor k;
        for(int ix=0; ix < nSquars; ++ix){
           for(int iy =0; iy <nSquars; ++iy){
        ...
        urect= QRect(posXofZoom * sizes::zoomLevel , posYofZoom * sizes::zoomLevel, nSquars * sizes::zoomLevel, nSquars * sizes::zoomLevel);
...
void MainWindow::rotation(int a)
{
    ...
    if(!sizes::isSelectionOn){
        ...
        areaSize();
        wArea->mPix = wArea->mPix.transformed(tf, Qt::SmoothTransformation);
        //showPix();
        PlaceEdges();
        raiseBorders();
    }
        ...
        selectionPix = sPix.scaled(sizes::selW / sizes::zoomLevel, sizes::selH/sizes::zoomLevel, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
    }
    updateInfo();
}
...
void MainWindow::on_autoRotationButton_clicked()
{
    ...
    if(ui->autoRotationButton->isChecked()){        
        ...
        preAngle =0;
        selectionPix = selectionRect->pixmap().scaled(sizes::selW/sizes::zoomLevel, sizes::selH/sizes::zoomLevel);
        updateInfo();
    }
}
...
void MainWindow::on_actionSizes_2_triggered()
{
    ...
    DialogSize dSize(this, wArea->mPix.width(), wArea->mPix.height());
    dSize.setModal(true);
    dSize.exec();
    sizes::areaWidth =  (dSize.pixWW * sizes::zoomLevel) + 8;
    sizes::areaHeight = (dSize.pixHH * sizes::zoomLevel) + 8;
...
void MainWindow::mirror(bool horizontal, bool vertical)
{
    ...
    if(sizes::isSelectionOn){
      ...
      save_previous(tr("Mirror ") + versus);
      wArea->mPix = QPixmap::fromImage(img);
      wArea->update();
    }
}
...
void MainWindow::on_actionCreate_mask_triggered()
{
    ...
    if(isSimil(sizes::activeColor, Qt::white, 45)) icolor = Qt::gray;
    for (int y = 0; y < (img.height()-1); ++y) {
        for (int x = 0; x < (img.width()-1); ++x) {
            ...
            if(abs(r-r2) > v || abs(g-g2)> v || abs(b-b2) > v || abs(r-r3) > v || abs(g-g3) > v || abs(b-b3) > v){
                img2.setPixelColor(x,y, sizes::activeColor);
            }else{
                img2.setPixelColor(x,y,icolor);
            }
        }
    }
    ...
    if(sizes::isSelectionOn){
        ...
        wArea->mPix = QPixmap::fromImage(img2);
        wArea->update();
    }
}
...
void MainWindow::on_actionCreate_triggered()
{
    ...
    save_area = new saveCam(wArea, QSize(preWidthCamera * sizes::zoomLevel, preHeightCamera * sizes::zoomLevel));
    int camx =ui->scrollArea->horizontalScrollBar()->value();
    int camy =ui->scrollArea->verticalScrollBar()->value();
    save_area->setGeometry(camx, camy, preWidthCamera * sizes::zoomLevel, preHeightCamera * sizes::zoomLevel);
    save_area->show();
    isCam = true;
}
...
{
    if(isCam){
        int ax = (save_area->pos().x() / sizes::zoomLevel);
        int ay = (save_area->pos().y() / sizes::zoomLevel);
        QPixmap camPix = wArea->mPix.copy(ax, ay, preWidthCamera, preHeightCamera);
        QFileDialog dialog(this);
        if(QDir(defaultPathDialog).exists() == false){defaultPathDialog = QDir::homePath();}
...
void MainWindow::on_historyCombo_activated(int index)
{
    ...
    if(prePix.isNull()) prePix = wArea->mPix;
    wArea->mPix = historyPix[index*2];  // 0-2-4 pixmap
...
{
  if(sizes::isSelectionOn){
        sizes::selH = sizes::selH / sizes::zoomLevel;
        sizes::selW = sizes::selW / sizes::zoomLevel;
        QImage img = selectionPix.toImage();
        sizes::selH *= 2.00;
        sizes::selW *= 2.00;
       ...
       // bigPix.fill(QColor(255, 255, 255, 0));
        ...
        for (int y = 0; y < img.height(); ++y) {
            ...
            for (int x = 0; x < img.width(); ++x) {
                ...
                nx = x * 2;
                ny = y * 2;
                bigImage.setPixelColor(nx, ny, k);
                bigImage.setPixelColor(nx+1, ny, k);
                bigImage.setPixelColor(nx, ny+1, k);
                bigImage.setPixelColor(nx+1, ny+1, k);
            }
        }
        sizes::selH = sizes::selH * sizes::zoomLevel;
        sizes::selW = sizes::selW * sizes::zoomLevel;
        selectionRect->resetGeometry();
        selectionPix = QPixmap::fromImage(bigImage);
        selectionRect->setPixmap(selectionPix);
  ...
  QPixmap bigPix(wArea->mPix.width() * 2, wArea->mPix.height() * 2);
  QImage bigImage = bigPix.toImage();
  QImage img = wArea->mPix.toImage();
  int nx=0;
  int ny =0;
  ...
  wArea->mPix = QPixmap::fromImage(bigImage);
  sizes::areaWidth = (wArea->mPix.width() * sizes::zoomLevel) + 8;
  sizes::areaHeight = (wArea->mPix.height() * sizes::zoomLevel) + 8;
  areaSize();
 // showPix();
...
void MainWindow::on_actionDivide_by_5_triggered()
{
    if(sizes::isSelectionOn){
        ...
        smallImage =  smallImage.convertToFormat(QImage::Format_ARGB32);
        QColor k;
        for (int y = 0; y < img.height()-1; y +=2) {

            for (int x = 0; x < img.width()-1; x +=2) {
                k = img.pixelColor(x, y);
                smallImage.setPixelColor(x/2, y/2, k);
            }
        }
        ...
        sizes::selW = sizes::selW * sizes::zoomLevel;
        selectionRect->resetGeometry();
        selectionPix = QPixmap::fromImage(smallImage);;
        selectionRect->setPixmap(selectionPix);
    }
    else{
    save_previous(tr("Divide by 4"));
    QPixmap smallPix(wArea->mPix.width() / 2, wArea->mPix.height() / 2);
    QImage smallImage = smallPix.toImage();
    QImage img = wArea->mPix.toImage();
    ...
    wArea->mPix = QPixmap::fromImage(smallImage);
    sizes::areaWidth = (wArea->mPix.width() * sizes::zoomLevel) + 8;
    sizes::areaHeight = (wArea->mPix.height() * sizes::zoomLevel) + 8;
    areaSize();
   // showPix();
...
void MainWindow::on_actionCreate_Line_triggered()
{
    ...
    if(lineD.res == 1 || lineD.res == 2){
        ...
        if(sizes::dotLines) pen.setStyle(Qt::DotLine);
        pai.setPen(pen);
        if(lineD.res == 1) {
...
void MainWindow::on_actionbase64_triggered()
{
    ...
    QPixmap camPix = wArea->mPix.copy(save_area->pos().x(), save_area->pos().y(), save_area->width(), save_area->height());
    QBuffer buffer;
...
void MainWindow::on_actionTo_Pdf_triggered()
{
    ...
    int offset = 0;
       while(offset < wArea->mPix.height()) {
           QRectF source(QPoint(0, offset), printer.pageRect(QPrinter::DevicePixel).size());
           painter.drawPixmap(printer.pageRect(QPrinter::DevicePixel), wArea->mPix, source);
           offset += printer.pageRect(QPrinter::DevicePixel).height();
           if(offset < wArea->mPix.height()){ printer.newPage();}
       }
    painter.end();
...
void MainWindow::on_actionStretch_area_triggered()
{
    ...
    if(strd->res == QDialog::Accepted){
        sizes::selH = double(strd->epix.height()) * sizes::zoomLevel;
        sizes::selW = double(strd->epix.width()) * sizes::zoomLevel;
        //save_previous(selectionCoords());
        selectionRect->resetGeometry();
...
{
    if(sizes::isSelectionOn){
        preCropped = wArea->mPix;
        save_previous(tr("Crop"));
        remSelX = (sizes::selX / sizes::zoomLevel) +0.99;
        remSelY = (sizes::selY / sizes::zoomLevel) +0.99;
        sizes::selX = 0;
        sizes::selY = 0;
...
void MainWindow::on_actionView_area_as_image_crop_triggered()
{
    remSelX = ui->scrollArea->horizontalScrollBar()->value() / sizes::zoomLevel;
    remSelY = ui->scrollArea->verticalScrollBar()->value() / sizes::zoomLevel;
    double aw = wArea->mPix.width() - remSelX;
    double ah = wArea->mPix.height()- remSelY;

    if(ui->scrollArea->width()/sizes::zoomLevel < aw) aw = ui->scrollArea->width()/sizes::zoomLevel;
    if(ui->scrollArea->height()/sizes::zoomLevel < ah) ah = ui->scrollArea->height()/sizes::zoomLevel;

    preCropped = wArea->mPix;

    wArea->mPix = wArea->mPix.copy(remSelX, remSelY, aw, ah);
    save_previous(tr("Crop"));
...
void MainWindow::addChart(QPixmap gr)
{
    ...
    int wImg = gr.width();
    int hImg = gr.height();
    int v = wArea->mPix.height() + 3;
    sizes::areaHeight += (double(gr.height()) * sizes::zoomLevel);
    if(wImg > wArea->mPix.width()){ sizes::areaWidth = (double(wImg) * sizes::zoomLevel) + 8.00; }
    reSize();
    QPainter p(&wArea->mPix);
    p.drawPixmap(4, v, gr);
    //showPix();
...
void MainWindow::on_actionSet_as_restore_point_triggered()
{
    if(toRestore.isNull()){
        ...
        int q = QMessageBox::question(this, "Drawish", tr("Do you want to overwrite the previous restore point?"), QMessageBox::Yes | QMessageBox::No);
        if(q == QMessageBox::Yes){
            QMessageBox::information(this, "Drawish", tr("Restore point overwritten"));
        }else{ return; }
    }
    toRestore = wArea->mPix;
...
void MainWindow::on_actionRestore_triggered()
{
    ...
    save_previous(tr("Restore"));
    wArea->mPix = toRestore;
    newImage("pix");
...
void MainWindow::on_actionCreate_shape_triggered()
{
    ...
    if(figure.center == true){
        ...
        QPainter pai2(&wArea->mPix);
        pai2.setRenderHint(QPainter::Antialiasing, sizes::aliasing);
        QPen pen2(ncol, 2);
        pai2.setPen(pen2);
        pai2.drawPoint(QPoint(sizes::shape_x_begin + xc+1, sizes::shape_y_begin + yc+1));
    }
    int correct= (sizes::line_width + 1) /2;
    urect =QRect(sizes::shape_x_begin-correct, sizes::shape_y_begin-correct, sizes::lineXEnd+sizes::line_width, sizes::lineYEnd+sizes::line_width);
    wArea->update(urect);
}
...
void MainWindow::on_actionSet_as_favorite_color_triggered()
{
    ...
    // save to image
    fileIO fio;
    fio.saveFav_color(fav, QDir::homePath() + "/Drawish_Data/drawish_3_fav_colors.png");
}
...
{
#ifdef Q_OS_APPLE
    QMessageBox::information(this, "Drawish", tr("Not supported for this OS!"));
#endif
    ...
    if(args.count() == 0){
        args.push_back(QCoreApplication::applicationFilePath());
        QMessageBox::information(this, "Drawish", args.at(0) + tr(" will be installed!"));
    }
...
{
    //   create icon
    QString pathIco=QDir::homePath() + "/.local/share/icons/drawish3.png";
    QDir myApp;
    QPixmap ikon(":/res/draw.png");
    if(!myApp.exists(QDir::homePath() + "/.local/share/icons")){
        myApp.mkdir(QDir::homePath() + "/.icons");
        pathIco = QDir::homePath() + "/.icons/drawish3.png";
        ikon.save(pathIco, "PNG");
    }
    else{
        if(QFile::exists(QDir::homePath() + "/.local/share/icons/drawish3.png")== false){
            ikon.save(QDir::homePath() + "/.local/share/icons/drawish3.png", "PNG");        }
    }
    //  create desktop file
    ...
    QMessageBox::information(this, "Drawish", tr("Drawish added to menu"));
}
...
{
    QDesktopServices::openUrl(QUrl("https://github.com/nikkNizz"));
}
...
void MainWindow::on_actionSerialize_2_triggered()
{
...
void MainWindow::zoomAll(double zm)
{
    ...
    if( preZoom != sizes::zoomLevel){
        ...
        if(isCam){
            int aax = save_area->pos().x() / preZoom * sizes::zoomLevel;
            int aay = save_area->pos().y() / preZoom * sizes::zoomLevel;
            int aaw = preWidthCamera * sizes::zoomLevel;
            int aah = preHeightCamera * sizes::zoomLevel;
            save_area->setGeometry(aax, aay, aaw, aah);
        }
...
void MainWindow::on_actionKeep_aspect_ratio_of_the_selection_toggled(bool arg1)
{
    sizes::keepSelAspectRatio = arg1;
}


void MainWindow::on_actionDot_lines_toggled(bool arg1)
{
    sizes::dotLines = arg1;
}

void MainWindow::on_actionCustom_shape_to_curve_toggled(bool arg1)
{
    sizes::shapeToCurve = arg1;
}

void MainWindow::on_actionShow_line_angle_indicator_toggled(bool arg1)
{
    sizes::lineAngleIndicator = arg1;
}

void MainWindow::on_actionInitial_shapes_dimension_triggered()
{
    sizes::init_w_shape = QInputDialog::getInt(this, "Drawish", tr("Set width"),80, 12, 200);
    sizes::init_h_shape = QInputDialog::getInt(this, "Drawish", tr("Set height"), 80, 12, 200);
}


void MainWindow::toPrinter(int A)
{
    QPrinter printer;

    QPainter painter;
    if(!painter.begin(&printer)){ QMessageBox::warning(this, "Drawish", "No printer!");}
    else{
        QImage img;
        if(A < 2){
            img = wArea->mPix.toImage();
        }
        else if(A == 2){
            if(sizes::isSelectionOn == false){
...
void MainWindow::on_actionHorizzontally_triggered()
{
    save_previous(tr("Split horizontally"));
    int xp = QInputDialog::getInt(this, "Drawish", tr("Enter the split point in percentage "), 0, 10, 90);
    xp = wArea->mPix.width()* xp / 100;
    QPixmap aPix(wArea->mPix.width()+6, wArea->mPix.height());
    aPix.fill(Qt::white);
    QPixmap piece1 =wArea->mPix.copy(0, 0, xp, wArea->mPix.height());
    QPixmap piece2 = wArea->mPix.copy(xp, 0, wArea->mPix.width()-xp, wArea->mPix.height());
    QPainter p(&aPix);
    p.drawPixmap(0,0, piece1);
    p.drawPixmap(xp+6,0, piece2);
    wArea->mPix = aPix;
    sizes::areaWidth += 6;
    reSize();
}
...
{
    save_previous(tr("Split vertically"));
    int xp = QInputDialog::getInt(this, "Drawish", tr("Enter the split point in percentage "), 0, 10, 90);
    xp = wArea->mPix.height()* xp / 100;
    QPixmap aPix(wArea->mPix.width(), wArea->mPix.height()+6);
    aPix.fill(Qt::white);
    QPixmap piece1 =wArea->mPix.copy(0, 0, wArea->mPix.width(), xp);
    QPixmap piece2 = wArea->mPix.copy(0, xp, wArea->mPix.width(), wArea->mPix.height()-xp);
    QPainter p(&aPix);
    p.drawPixmap(0,0, piece1);
    p.drawPixmap(0,xp+6, piece2);
    wArea->mPix = aPix;
    sizes::areaHeight += 6;
    reSize();
}
...
void MainWindow::on_actionNew_window_triggered()
{
    ...
    myProcess->start(QCoreApplication::applicationFilePath());
}
...
void MainWindow::on_actionPixel_to_active_color_in_selection_triggered()
{
    ...
    for(int xx= 0;  xx < sw; ++xx){
        for(int yy=0; yy < sh; ++yy){
            if(selectedImage.pixelColor(xx, yy) == sizes::activeColor){
                selectedImage.setPixelColor(xx, yy, underImage.pixelColor(xx, yy));
            }else{
                selectedImage.setPixelColor(xx, yy, sizes::activeColor);
            }
        }
    }
    selectionPix = QPixmap::fromImage(selectedImage);
...
{
    sizes::roundAngle = checked;
}


void MainWindow::on_actionLine_round_cap_triggered(bool checked)
{
    sizes::lineRound = checked;
}


void MainWindow::on_mouseCorrectlSlider_valueChanged(int value)
{
    sizes::mouseCorrection = value;
}


void MainWindow::on_markerButton_toggled(bool checked)
{
    sizes::semitrasp = checked;
    if(sizes::isCurveLineAreaOn){
        if(checked){    cl_area->trsp = 128;}
        else{           cl_area->trsp = 255;  }
        cl_area->createPen();
    }
}


void MainWindow::on_actionAnti_aliasing_smooth_lines_triggered(bool checked)
{
    sizes::aliasing = checked;
}


void MainWindow::on_actionSet_limit_for_undo_triggered()
{
    maxUndo = QInputDialog::getInt(this, "Drawish", tr("Set the undo limit (1-100)"), maxUndo, 1, 100);
}


void MainWindow::on_trSelectionSlider_valueChanged(int value)
{
    if(sizes::isSelectionOn){
    QImage img = selectionPix.toImage();
      img =  img.convertToFormat(QImage::Format_ARGB32);
    ...
    QPixmap tPix = QPixmap::fromImage(img);
    selectionRect->setPixmap(tPix);
    }
}


void MainWindow::on_trSelectionSlider_sliderReleased()
{
    if(sizes::isSelectionOn){
    selectionPix = selectionRect->pixmap();
    selectionRect->setPixmap(selectionPix);
    }
}


void MainWindow::on_ViewportToImg_clicked()
{    
    on_actionView_area_as_image_crop_triggered();    
}


void MainWindow::on_actionShow_button_view_toggled(bool arg1)
{
