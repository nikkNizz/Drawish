...
void MainWindow::drawWithPen(){
    ...
    if(sizes::penType == 3 || sizes::penType == 4){
        ...
        QImage img1 = freeSelPix.toImage();
        QImage img2 = pxm.toImage();
        int r1, r2, g1, g2, b1, b2;
        int strong = 2;
        for(int xx =0; xx < sizes::line_width; ++ xx){
            for(int yy =0; yy < sizes::line_width; ++yy){
                double hyp = sqrt((corners - xx)*(corners - xx) + (corners - yy)*(corners-yy) );
                if(hyp < double(corners) +1.00){
                   strong = 1; // you are in the area of circle (more or less)
                }else{
                    strong = 4;
                }
                r1 = img1.pixelColor(xx, yy).red();
                r2 = img2.pixelColor(xx, yy).red();
                r1 = (r1 + r2*strong) /(strong+1);
                g1 = img1.pixelColor(xx, yy).green();
                g2 = img2.pixelColor(xx, yy).green();
                g1 = (g1 + g2*strong) / (strong +1);
                b1 = img1.pixelColor(xx, yy).blue();
                b2 = img2.pixelColor(xx, yy).blue();
                b1 = (b1 + b2*strong) / (strong +1);
                img1.setPixelColor(xx, yy, QColor( r1, g1, b1));
            }
        }
        freeSelPix = QPixmap::fromImage(img1);
        QPainter pai(&wArea->mPix);
        ...
        for(int i=0; i < maxIter; ++i){
            pai.drawPixmap((x1+factorx*i)-ray, (y1 + factory*i)-ray , QPixmap::fromImage(img));
       ...
       pai.setPen(pen);
           ...
           double factory = double(stepy) / maxIter;
           for(int i=0; i < maxIter; ++i){
               pai.drawPoint(x1+factorx*i, y1 + factory*i );
               if(sizes::penType == 2){ //highlight
                  pai.setPen(pen2);
                  pai.drawPoint(x1+factorx*i, y1 + factory*i );
               }
