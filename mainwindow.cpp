    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    version_info = "2.7";
    // 2.6: fixed borders in textArea (( mainwindow.cpp )); label in stretch dialog (( stretchdialog.ui ))
    // new or changed tooltips, (( mainwindow.ui ))       ; add favourites fonts ((mainwindow.h)) ;
    // message before overwrite restore point             ; keep sel asp. ratio ((geometric.h)) ((geometric.cpp)) ((selectionarea.cpp))
    // edit menu -> options  add separators in image menu ; dot lines ((curvelinearea.cpp))
    // strong color in tracer ((tracer.cpp))((tracer.h))  ; coords  in msgbox((zoomarea.h)) ((zoomarea.cpp))
    // removed flat8 --> added digital marker             ; custom shapes with curve option((shapearea.cpp))
    // Added labels                                       ; blur slider ((dialogeffects.ui))
    // prevented from bringing the dimensions to zero ((angolo.cpp)) ((borderbottom.cpp)) ((borderright.cpp))
    // Removed a checkbox ((dialogeffects.cpp))           ; removed paste over file -> added reattach
    // bugfix in info (zoomlevel)  and label position     ; line angle indicator ((curvelinearea.h)) & config
    // alt+R reattach
    //---------------------------------------------------------------------------------------------
    //2.7  tracer bugfix  ;  selection show size when rubber; viewport to image; smudge pen; pencil
    // alt+1 view;  alt-  crop  ;   add info in line indicator;  bug in curvelinearea (right click disabled)
    // auto save; alt+O alt+V; better tracer colors
...
QPen MainWindow::configPen(QColor &ncol, int alpha)
{
    ...
    else if(jColor ==1){ pen1.setCapStyle(Qt::SquareCap);}
...
{
    if(ui->comboPen->currentIndex() > 2 && ui->comboPen->currentIndex() < 5 ){
        // stylus dont draw 1st point
        save_previous(tr("Stylus pen"));
        return;
    }
    if(ui->comboPen->currentIndex() == 9){
    ...
    if(ui->comboPen->currentIndex() == 8){// labels
        nextLabel++;
        pen.setWidth(3);
