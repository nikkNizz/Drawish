    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    version_info = "2.9";
    // 2.9 improved shape area    ;  div shape and text css; [bug] not frameless when move textarea
    // option step angle 5°       ; alt+< for draw text    ; new icons;   Moved serialize menu
    // [bug] in save previous text; [bug] pdf length       ;
    setGeometry(50,80,790,487);

    isLinux = false;
#ifdef Q_OS_LINUX
    isLinux = true;
    ui->actionDesktop_shortcut->setText("Add to menu");
#endif

    ui->textOptionsWidget->setVisible(false);
    ui->lineOptionWidget->setGeometry(80,0,610,31);
...
void MainWindow::resizeEvent(QResizeEvent *event)
{
    ...
    ui->infoWidget->setGeometry(80,this->height()-88, 660, 60);
}
...
void MainWindow::updateInfo()
{
  ...
  ui->mouseX_label->setNum(sizes::selX / zoomLevel);
  ui->mouseY_label->setNum(sizes::selY / zoomLevel);
  if(sizes::isSelectionOn || sizes::isShapeOn){
    ui->selX_label->setNum(int(sizes::selW / zoomLevel));
    ui->selY_label->setNum(int(sizes::selH / zoomLevel));
  }else{
...
void MainWindow::on_actionReattach_triggered()
{
    ...
    pai.drawPixmap(remSelX, remSelY, pix);
    pix = preCropped;
    remSelX = -123;
    remSelY = -145;
    newImage("pix");
...
{
    // combopen: 0 round, 1 square, 2 highlight, 3 slash, 4 backslash, 5 fusion, 6 random 7 eraser
    ...
    if(jColor ==6 ){ // random
        ...
        int a = (sizes::selX / zoomLevel) -df;
        int c = (sizes::selY / zoomLevel) -df;
    ...
    if(jColor == 3 || jColor == 4 ) pen1.setWidth(1);
    else if(jColor ==1 || jColor == 11){ pen1.setCapStyle(Qt::SquareCap);}
...
void MainWindow::drawWithPen(){
    ...
    if(penIndex == 3 || penIndex == 4){
    ...
    else if(penIndex == 9){       // smudge
        int corners = sizes::line_width / 2;
        QPixmap pxm =pix.copy(sizes::shape_x_end/zoomLevel-corners, sizes::shape_y_end/zoomLevel-corners, sizes::line_width, sizes::line_width);
        ...
        for(int xx =0; xx < sizes::line_width; ++ xx){
            for(int yy =0; yy < sizes::line_width; ++yy){
                ...
                if(hyp < double(corners) +1.00){
                   strong = 2; // you are in the area of circle (more or less)
                }else{
                    strong = 4;
                }
        ...
        QPainter pai(&pix);
        pai.drawPixmap(sizes::shape_x_end/zoomLevel-(corners), sizes::shape_y_end/zoomLevel-(corners), freeSelPix);
        pai.end();
        freeSelPix = pix.copy(sizes::shape_x_end/zoomLevel-corners, sizes::shape_y_end/zoomLevel-corners, sizes::line_width, sizes::line_width);
    }
    else if(penIndex == 13 || penIndex == 14){
        int corners = sizes::line_width / 2;
        QPixmap pxm =pix.copy(sizes::shape_x_end/zoomLevel-corners, sizes::shape_y_end/zoomLevel-corners, sizes::line_width, sizes::line_width);
        QImage img2 = pxm.toImage();
        QColor k;
        ...
        QPainter pai(&pix);
        pai.drawPixmap(sizes::shape_x_end/zoomLevel-(corners), sizes::shape_y_end/zoomLevel-(corners), QPixmap::fromImage(img2));
        pai.end();
    }

    else if(penIndex == 10){ // pencil
        QPainter pai(&pix);
        ...
        for(int h =0; h < sizes::line_width; ++h){
            for(int w =0; w < sizes::line_width; ++w){
                ...
                if(hyp < double(ray) + 0.51){   // you are in the area of circle
                    nRand =QRandomGenerator::global()->generate() % 80;
                    nRand += 70;
                    if(ui->markerButton->isChecked()){alfa = 32;}
                    else{ alfa =255;}
                    img.setPixelColor(w, h, QColor(nRand, nRand, nRand, alfa));
                }
            }
        }
        int x1 = sizes::shape_x_begin / zoomLevel;
        int y1 = sizes::shape_y_begin / zoomLevel;
        int x2 = sizes::shape_x_end / zoomLevel;
        int y2 = sizes::shape_y_end / zoomLevel;
        int stepx = x2-x1;
        int stepy = y2-y1;
...
void MainWindow::draw_first_point()
{
    ...
    int comboPos = ui->comboPen->currentIndex();

    if(comboPos > 2 && comboPos < 5 ){
    ...
    if(comboPos == 9){
        ...
        int corners = sizes::line_width / 2;
        freeSelPix = pix.copy(sizes::shape_x_begin/zoomLevel-corners, sizes::shape_y_begin/zoomLevel-corners, sizes::line_width, sizes::line_width);
        return;
    }
    else if(comboPos == 13){ save_previous("Lighten"); return;}
    else if(comboPos == 14){ save_previous("Darken"); return;}
    ...
    if(comboPos == 11){ save_previous(tr("Eraser"));}
    ...
    if(comboPos == 8){ // labels
        ...
        pai.setFont(fo);
        pai.drawLine(sizes::shape_x_begin / zoomLevel,sizes::shape_y_begin/zoomLevel, sizes::shape_x_begin / zoomLevel+ 8, sizes::shape_y_begin / zoomLevel-8 );
        pai.drawText(sizes::shape_x_begin / zoomLevel+10, sizes::shape_y_begin / zoomLevel-10, QString::number(nextLabel));
    }else{
...
void MainWindow::on_conn_Curve_clicked()
{
    ...
    if(ui->conn_Curve->isChecked()){
        ...
        cl_area = new curveLineArea(wArea, zoomLevel, t);
...
void MainWindow::save_previous(QString tx)
{
    ...
    if(ui->actionSave_to_disk_every_10_changes->isChecked() && countChanges > 9){
        countChanges =0;
        // save to disk
        pix.save(QDir::homePath() + "/Drawish_Data/tmp_pix1.png");
    }
    historyCount = 1;
...
bool MainWindow::isSimil(QColor k1, QColor k2, int affinity)
{
  ...
  if(diffr > affinity*2.5 )return false;
  return true;
}
...
{
    sizes::selW = sizes::init_w_shape * zoomLevel;
    sizes::selH = sizes::init_h_shape * zoomLevel;
...
void MainWindow::on_shapeToSel_clicked()
{
    ...
    QPixmap spix = shape_area->pixmap();
    int sx = shape_area->x()/zoomLevel;
    int sy = shape_area->y()/zoomLevel;
    int sw = shape_area->width()/zoomLevel;
    int sh = shape_area->height()/zoomLevel;
    QImage img = spix.toImage().scaled(sw, sh);
    QImage ipix = pix.toImage();
    QColor k;
...
void MainWindow::on_actionTo_Pdf_triggered()
{
    ...
    int offset = 0;
       while(offset < pix.height()) {
           ...
           if(offset < pix.height()){ printer.newPage();}
       }
    painter.end();
...
{
    remSelX = ui->scrollArea->horizontalScrollBar()->value() / zoomLevel;
    remSelY = ui->scrollArea->verticalScrollBar()->value() / zoomLevel;
    double aw = pix.width() - remSelX;
    double ah = pix.height()- remSelY;

    if(ui->scrollArea->width()/zoomLevel < aw) aw = ui->scrollArea->width()/zoomLevel;
    if(ui->scrollArea->height()/zoomLevel < ah) ah = ui->scrollArea->height()/zoomLevel;

    preCropped = pix;

    pix = pix.copy(remSelX, remSelY, aw, ah);
    save_previous("Crop");
...
void MainWindow::on_actionRedo_only_changes_triggered()
{
    // prepix is newer
    if(!prePix.isNull()){
        if(sizes::isSelectionOn){
        ...
        int w1 = qMax(prePix.width(), pix.width());
        int h1 = qMax(prePix.height(), pix.height());
        QPixmap wpix(w1, h1);
        wpix.fill(Qt::white);
        QPainter p(&wpix);
        ...
        pix = QPixmap::fromImage(imgPrePix);
        pix = pix.copy(0,0, imgPrePix.width(), imgPrePix.height());
        newImage("pix");
}
}
...
void MainWindow::on_actionHorizzontally_triggered()
{
    ...
    xp = pix.width()* xp / 100;
    QPixmap aPix(pix.width()+6, pix.height());
    aPix.fill(Qt::white);
    QPixmap piece1 =pix.copy(0, 0, xp, pix.height());
    QPixmap piece2 = pix.copy(xp, 0, pix.width()-xp, pix.height());
    QPainter p(&aPix);
...
void MainWindow::on_actionVertically_triggered()
{
    ...
    int xp = QInputDialog::getInt(this, "Drawish", tr("Enter the split point in percentage "), 0, 10, 90);
    xp = pix.height()* xp / 100;
    QPixmap aPix(pix.width(), pix.height()+6);
    aPix.fill(Qt::white);
    QPixmap piece1 =pix.copy(0, 0, pix.width(), xp);
    QPixmap piece2 = pix.copy(0, xp, pix.width(), pix.height()-xp);
    QPainter p(&aPix);
    p.drawPixmap(0,0, piece1);
