    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    version_info = "2.5";
    // 2.5: when spray, show pen width; mainwindow ui; edit similarity; shape to selection;
    // [Linux: update notification in appRun (appimage)];
...
{
    // combopen: 0 round, 1 square, 2 flat, 3 slash, 4 backslash, 5 fusion, 6 random 7 eraser
    ...
    else if(jColor ==2){pen1.setCapStyle(Qt::FlatCap); pen1.setWidth(8);}
...
{
    if(ui->comboPen->currentIndex() > 1 && ui->comboPen->currentIndex() < 5){
        // flat8, stylus dont draw 1st point
