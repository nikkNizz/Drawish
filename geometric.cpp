#include "geometric.h"

double::sizes::zoomLevel = 1.0;
QString sizes::passedFile ="";
int sizes::areaHeight=328;
int sizes::areaWidth= 488;
bool sizes::startResize=false;
int sizes::activeOperation=0;
int sizes::selX=0;
int sizes::selY=0;
float sizes::selW=10.0;
float sizes::selH=20.0;
bool sizes::isSelectionOn=false;
bool sizes::isShapeOn=false;
bool sizes::modify = false;
bool sizes::isLinux = false;
QColor sizes::activeColor=QColor(0,0,0);
int sizes::line_width=6;
int sizes::shape_x_begin=40;
int sizes::shape_y_begin=40;
int sizes::shape_x_end=0;
int sizes::shape_y_end=0;
QString sizes::activeShape="squ";
int sizes::FillStyle = 0;
bool sizes::isCurveLineAreaOn = false;
int sizes::zoomx =0;
int sizes::zoomy =0;
bool sizes::zoomEdited=false;
int sizes::lineXEnd = 0;
int sizes::lineYEnd = 0;
int sizes::init_w_shape = 80;
int sizes::init_h_shape = 80;
int sizes::curveStretch = 0;
QString sizes::savedRtf = "";
int sizes::mouseCorrection = 1;
bool sizes::isArrow = false;
bool sizes::isRotating = false;
QString sizes::selectData ="0 0 0 0";
QList<QPoint> sizes::freeSelPoints;
QPoint sizes::autoPoint=QPoint(-1,-1);
bool sizes::keepSelAspectRatio = false;
bool sizes::dotLines = false;
QList<int> sizes::customShapesCoord;
bool sizes::shapeToCurve = false;
bool sizes::lineAngleIndicator = true;
bool sizes::roundAngle = false;
bool sizes::lineRound = true;
bool sizes::aliasing = false;
bool sizes::semitrasp = false;
int sizes::penType = 0;
